...
public class TradeMain extends Thread {
    ...
    public void handleMessage(String message){
        ...
        if (tInfo.getStatus() == true) {	// 거래 시작 (DB에 거래 정보 입력)
        	...
        	tInfo.updateDB(String.format("update trade set on_going=0, end_date=\'%s\' where user_id=\'%s\' and bot_name=\'%s\' and on_going=1" ,
        			dateFormat.format(new Date()), tInfo.getId(), tInfo.getName()));
        }
    }
