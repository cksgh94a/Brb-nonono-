...
class IndicatorFunction_bt {
	...
	public static double[][] get_HLCV_HistoryArray(String exchange, String coin, String base, int interval,
		...
		// startUt ~ endUt 만큼의 OHLC데이터를 디비에서 가지고 옴
		
		// 아래는 각 디비에 맞는 설정
		String symb;
		if (exchange.equals("binance") || exchange.equals("hitbtc") || exchange.equals("BINANCE")
		...
		// 기간 : startUt - endUt 만큼의 기간을 구함 - 사용안함 
		long period = (endUt - startUt) / interval;

		DB_ohlc db = new DB_ohlc();
		ResultSet rs = null;

		// startUt부터 ~ endUt까지의 데이터를 뽑아오는 코드 : 시간기준
		...
		///////////////////////////////////////////// 매우중요
		///////////////////////////////////////////// 매우중
		double ret[][] = new double[size - 1][5]; // SIZE - 1 ?????????????????? 를 해줘야 한다! 
		// 왜그런지는 rs.last 와 rs.first의 기능을 잘 모르겠다. rs가 행 갯수 + 1 만큼 가지고 오는건지? (rs.next를 보아할 땐 그럴 거 같기도)
		int cnt = 0;
		try {
			while (rs.next()) {
